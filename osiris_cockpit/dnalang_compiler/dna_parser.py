...
class Lexer:
    ...
    def skip_whitespace(self):
        ...
        while ch is not None and ch in ' \t\r':
            self.advance()
            ch = self.current_char()
...
class Parser:
    ...
    def parse_gene(self) -> GeneNode:
        ...
        # Parse encoding expression — 'encode' is a keyword, accept both
        encoding_token = self.current_token()
        if encoding_token.type in (TokenType.IDENTIFIER, TokenType.KEYWORD):
            self.advance()
        else:
            raise SyntaxError(f"Expected encode, got {encoding_token.type} at {encoding_token.line}:{encoding_token.column}")
...
def parse_canon_block(canon_text: str) -> List[rDNAEntry]:
    ...
    """
    entries = []
    blocks = canon_text.split(CANON_SEPARATOR)
    
    for i, block in enumerate(blocks):
        # Skip initial header block
        if CANON_PREFIX not in block:
            continue
        
        # Extract Canon Name
        try:
            name_line = block.split('\n', 1)[0].strip()
            name_match = re.search(r'Canon ([IVXLCDM]+) — ([A-Z-]+)', name_line)
            if name_match:
                canon_name = f"{name_match.group(1)}_{name_match.group(2)}"
            else:
                canon_name = f"UNKNOWN_CANON_{i}"
        except IndexError:
            continue
        ...
        # Parse fields
        for match in re.finditer(FIELD_PATTERN, block, re.MULTILINE):
        ...
        if field_data:
            entry = rDNAEntry(
                ...
                directive_raw=field_data.get("Directive", ""),
                evolution_vector=field_data.get("Evolution Vector", ""),
                coherence_target=field_data.get("Coherence Target", "")
            )
